...
4. 启动，加载所有自动配置类 `xxxAutoConfiguration`
   ...
   - c. **属性类**和**配置文件**前缀项绑定
   - d. `@Conditional` 派生的条件注解进行判断是否组件生效
5. 效果：
   - a. 修改配置文件，修改底层参数
   - b. 所有场景自动配置好直接使用
...
在SpringBoot中，使用 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`
...
| SpringBoot生命周期过程 | Spring容器刷新过程 | 自动配置生效 |
| --- | --- | --- |
| starting（引导启动 BootstrapContext） | prepareRefresh（环境准备） | @EnableAutoConfiguration |
| environmentPrepared（环境准备完成） | obtainFreshBeanFactory（获取工厂） | @AutoConfigurationPackage 扫描主程序所在包 |
| | prepareBeanFactory（准备工厂） | @Import AutoConfigurationImportSelector 导入自动配置 |
| | postProcessBeanFactory（预留处理） | 准备工厂的时候加载自动配置类 |
| contextPrepared（ioc创建、准备完成，主程序未加载） | invokeBeanFactoryPostProcessors（工厂可扩展后置处理机制） | 扫描主程序以及子包 |
| contextLoaded（ioc加载，并未刷新） | registerBeanPostProcessors（注册Bean后置处理器） | |
| | initMessageSource（初始化消息） | |
| started（ioc刷新，runner未调用） | initApplicationEventMulticaster（初始化事件派发） | |
| | onRefresh（预留接口，子类回调） | |
| ready（ioc刷新，runner调用完成） | registerListeners（注册监听器） | @Configuration, @Component, @Bean, @Controller |
| | finishBeanFactoryInitialization（完成初始化，所有组件创建） | @Service, @Repository |
| running（程序运行） | finishRefresh（刷新完成） | 业务组件生效 |
